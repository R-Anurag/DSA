# [Peak element in Array - Tutorial](https://takeuforward.org/data-structure/peak-element-in-array) 
 _https://takeuforward.org/data-structure/peak-element-in-array_
...
**Note:** _For the first element, the previous element should be considered as negative infinity as well as for the last element, the next element should be considered as negative infinity._
**Pre-requisite:** [Binary Search Algorithm](https://takeuforward.org/data-structure/binary-search-explained/)
**Examples**
Example 1:
...
[Brute Force Approach](#brute-force-approach) [Optimal Approach](#optimal-approach)
[](#dsa_article_video_explanation)
...
**Dry-run:** _Please refer to the_ [_video_](https://youtu.be/cXxmbemS6XM) _for the dry-run._
...
Output: The peak is at index: 7Complexity Analysis
...
1. **If arr\[i\] > arr\[i-1\]:** we are in the left half.
2. **If arr\[i\] > arr\[i+1\]:** we are in the right half.
...
### **Algorithm:**
**Note:** _At the start of the algorithm, we address the edge cases of identifying the peak element without requiring separate conditions during the check for arr\[mid\] inside the loop. And the search space will be from index 1 to n-2 as indices 0 and n-1 have already been checked in the edge cases._
The final steps will be as follows:
1. **If n == 1:** This means the array size is 1_._ If the array contains only one element, we will return that index i.e. 0.
...
6. **Check if arr\[mid\] is the peak element:****If arr\[mid\] > arr\[mid-1\] and arr\[mid\] > arr\[mid+1\]:** If this condition is true for arr\[mid\], we can conclude arr\[mid\] is the peak element. We will return the index ‘mid’. 
...
Video Explanation
> _Special thanks to **[KRITIDIPTA GHOSH](https://www.linkedin.com/in/kriti39/)**_ _for contributing to this article on takeUforward._
